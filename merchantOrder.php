...
<?php 
	...
	$sql = "SELECT address, transaction_hash, id, confirmed_hash FROM record WHERE m_address='$address'";
	...
	?>
		<div style="background-color:#<?php echo $color; ?>;padding:10px;margin-bottom:5px;">
			<?php 
				...
				echo "<h2>已確認Hash:</h2>"; 
				echo "<p id=".$id.">".$row["confirmed_hash"]."</p>";
			...
			<!--這裡藏了一個表單，可以用來傳遞交易id-->
			<input hidden name="record_id" value=<?php echo $row["address"];?>>
			<input type="submit" name="uniqueUserRecord" style="width:300px" value="檢視與該用戶的交易紀錄"/>
			</form>
			<br>
		</div>
	<?php
